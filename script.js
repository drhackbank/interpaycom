    ...
    for(let i=0;i<particles.length;i++){
      for(let j=i+1;j<particles.length;j++){
        ...
        if(d<120){
          ...
          ctx.strokeStyle=`rgba(0,112,243,${.15*(1-d/120)})`;ctx.lineWidth=.5;ctx.stroke();
        }
      }
    }
...
function doLogout(){
  ...
  clearLErr();_payInit=false;_ppLoaded=false;_ppClientId='';
}
...
function updateSummary(){
  ...
  if(ppEl&&a>0&&PP_UNSUPPORTED.includes(c.code)){
    ppEl.textContent=`PayPal will charge: $${(a/c.rate).toFixed(2)} USD`;ppEl.style.display='block';
...
async function loadPayPalSDK(){
  ...
  try{
    ...
    if(!d.clientId||d.clientId.length<8){
      if(g('pp-loading'))g('pp-loading').innerHTML='<span style="color:var(--err);font-size:12px;text-align:center;line-height:1.8">PayPal not configured.<br/><small>Add PAYPAL_CLIENT_ID in Vercel env vars.</small></span>';
      return;
    }
...
function injectPPSDK(clientId){
  ...
  s.onerror=()=>{
    if(g('pp-loading'))g('pp-loading').innerHTML='<span style="color:var(--err);font-size:12px">PayPal SDK failed to load.</span>';
  };
  ...
  window.paypal.Buttons({
    ...
    onApprove:async(data)=>{
      ...
      try{
        ...
        showMsg('err',e.message||'Payment capture failed. Please contact support.');
      }
    },
    onError:(err)=>{
      console.error('[PayPal]',err);
      showMsg('err','PayPal encountered an error. Please try again.');
    },
    onCancel:()=>{
      showMsg('warn','Payment cancelled. You can try again anytime.');
    },
  }).render('#pp-buttons');
}
...
function resetPayment(){
  g('success-card').classList.add('hidden');
  g('pay-card').classList.remove('hidden');
